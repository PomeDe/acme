...
export default function Home() {
    ...
    return array.filter(
      (el) => (el.name).toLowerCase().includes(val.toLocaleLowerCase())
    )
  }
  ...
  <img src="https://img.icons8.com/ios7/512/FFFFFF/menu--v3.png"/>
        ...
        <div className="flex flex-row justify-center space-x-25 text-white w-full">
...
<li className="mt-2 flex text-white"><a className="w-full text-sm underline-offset-4 hover:underline dark:hover:text-neutral-100 " href="/search/stickers">Stickers</a></li>
            ...
            <div className=" lg:grid-cols-3  gap-5 sm:grid-cols-2 grid ">
                {filtered.length >0?(filtered.map((element)=>(
                     <Link
                     href={`/product/${element.id}`}
              key={element.id}
              className="group relative bg-black rounded-md shrink-0 lg:w-90 sm:w-70 lg:h-80 sm:h-60 flex justify-center items-center overflow-hidden cursor-pointer hover:border-2 hover:border-blue-500 transition duration-300 ease-in-out"
            >
              <img
                src={element.image}
                alt={element.name}
                className="lg:w-2/3 sm:w-1/2 sm:1/2 lg:h-2/3 object-cover transition-transform duration-300 group-hover:scale-110"
              />
              <div className="absolute bottom-4 left-4 flex items-center space-x-3 bg-black/70 border border-gray-500 rounded-3xl h-10 px-3 text-xs">
                ...
                <div className="bg-blue-600 rounded-3xl px-3 py-1 font-semibold">
                  {element.price}
                    ...
                    <p>There are no products that match "{val}"</p>
            </div>)}
            </div>
